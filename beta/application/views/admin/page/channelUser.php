...
<div class="container-fluid p-0">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">DSA Registration</li>
    </ol>
    </nav>
</div>
			...
			<table class=" table-bordered text-center table-hover " id="dtBasicExample">
				...
				<tbody id="appBody">
					...
					foreach($datas as $data) {
                        ...
                        $parent_name = '';
                         if ($data->parent_id != '' && $data->parent_id != 0 && $data->parent_id != NULL) {
                            $parent_name = $this->db->where('id',$data->parent_id)->where('role', $data->parent_id_role)->get('user_master')->row('name');
                            if (empty($parent_name)) {
                                $parent_name = $this->db->where('id',$data->parent_id)->where('role', $data->parent_id_role)->get('branch_franchise')->row('name');
                            }
                        }else {
                             $parent_name = 'Admin';
                        }
					...
					<tr>
						...
						<!-- <td class=''><?php echo ucwords(date('d-m-Y h:i A', strtotime($data->created_on))); ?></td> -->
						
						<td class=''><?php  if ($data->status == 1) {echo 'Active';} else {echo 'Inactive';} ?></td>

                        <td>
                            <div class="d-flex justify-content-center align-items-center" style="gap: 8px;">
                                <a href="<?php echo base_url('admin/Dashboard/userProfiledetail/').$data->id;?>" class="icon-btn" title="View Profile">
                                    <i class="fa fa-user"></i>
                                </a>
                                <?php if($data->parent_id_role != 1) {?>
                                <a href="<?php echo base_url('admin/agreement/'.$data->id.'/'.$data->role);?>" class="icon-btn" title="View Agreement">
                        ...
                        <td>
                            <div class="d-flex justify-content-center align-items-center" style="gap: 8px;">
                                ...
                                <a href="<?php echo base_url('admin/edit-partner/').$data->id;?>" class="icon-btn" title="Edit">
                                    <i class="fa fa-pencil-square-o"></i>
                                </a>
                                <a href="#" class="icon-btn btn-danger" title="Delete" onclick="delUser(<?= $data->id ?>)">
                                    <i class="fa fa-trash"></i>
                                </a> 
                                 <?php }?>
                                <a href="#" class="icon-btn" title="Change Status" onclick="statusUser(<?= $data->id ?>, <?= $data->status ?>)">
                                    <i class="fa fa-eye"></i>
                                </a>
                            </div>
                        </td>
