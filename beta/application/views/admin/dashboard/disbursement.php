...
<div class="container-fluid p-0">
     <nav aria-label="breadcrumb">
        <ol class="breadcrumb ">
           ...
           <li class="breadcrumb-item active" aria-current="page"><?=ucwords($_GET['role']);?> Disbursement Leads</li>
       </ol>
     </nav>
</div>
			...
			<table class=" table-bordered text-center table-hover" id="dtBasicExample">
					...
					<?php
                       ...
                       $role = isset($_GET['role']) ? $_GET['role'] : '';
                        ...
                        if ($role === 'paper' && $type === 'team') {
                            $datas = $team_disbursemenets_lead_paper;

                        } elseif ($role === 'digital' && $type === 'team') {
                            $datas = $team_disbursemenets_lead_digital;

                        } elseif ($role === 'paper') {
                            $datas = $disbursemenets_lead_paper;

                        } else {
                            $datas = $disbursemenets_lead_digital;
                        }
                        ...
                        if (!empty($datas)) {
                            ...
                            foreach ($datas as $data) {
                               $process = $this->db->where('id', $data->process_id)
                                                    ->get('loan_process')
                                                    ->row();

                                // MAIN USER FETCH (user_master → branch_franchise)
                                $user = $this->db->where('id', $data->uid)
                                                ->where('role', $data->uid_role)
                                                ->get('user_master')
                                                ->row();
                                ...
                                if (!empty($parent_id)) {
                                    ...
                                    // Fetch Parent Name: user_master → branch_franchise
                                    ...
                                    // Branch Franchise / DSA
                                    $branch_dsa = $parent_name;

                                    // My Network (Silver / Platinum)
                                    if ($subscription == "Silver" || $subscription == "Platinum") {
                                        $network_member = $parent_name;
                                    }

                                    // My Team (Other Subscriptions)
                                    if ($subscription != "Silver" && $subscription != "Platinum") {
                                        $team_member = $parent_name;
                                    }
                                }
                    ?>
                ...
                <tr>
                    ...
                    <!-- <td><?=$data->lead_status;?></td> -->
                     ...
                     <?php  if (!empty($parent_id) && $this->session->userdata('role') != 1) {?>
                        <td><?= $data->disbursed_team;?></td>
                        <?php }else{ ?>
